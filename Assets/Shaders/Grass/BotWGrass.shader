Shader "Custom/BotWGrass"
{
    Properties
    {
        ...
        _RootColor("Root Color Low", Color) = (1, 1, 1, 1)

        [Header(Blade Color Settings)]
        _TipColor("Tip Color", Color) = (1, 1, 1, 1)

        [Header(Blade Dimensions)]
        _BladeWidthMin("Blade Width (Min)", Range(0, 0.4)) = 0.02
        _BladeWidthMax("Blade Width (Max)", Range(0, 0.4)) = 0.05
		...
		_TessellationGrassDistance("Tessellation Grass Distance", Range(0.01, .2)) = 0.075
        _GrassMinHeightThreshold("Grass Min Height Threshold", Range(0, 1)) = 0.5
        _GrassMaxHeightThreshold("Grass Max Height Threshold", Range(0, 1)) = 0.8
        _TerrainHeightMultiplier("Terrain Height Multiplier", Float) = 6
		_GrassDistanceVisibility("Grass Distance Visibility", Float) = 90
		_TerrainSlopeThreshold("Terrain Slope Threshold", Range(0, 1)) = 0.5

        [Header(Noise Settings)]
        _NoiseScale("Noise Scale", Range(0, 2)) = 1
        _NoiseThreshold("Noise Threshold", Range(0, 1)) = 0.5
        [IntRange] _NoiseOctaves("Noise Octaves", Range (1,16)) = 1
        _NoiseHeightInfluence("Noise Height Influence", Range(0, 2)) = 0.5
        _NoiseHeightBias("Noise Height Bias", Range(0, 1)) = 0.5
        [Toggle] _INVERT_NOISE("Invert Noise", Range(0, 1)) = 0
        ...
        [Toggle] _MAIN_LIGHT_SHADOWS("Main Light Shadows", Float) = 0

        [Header(Additional Noise Settings)]
		[Toggle] _ADDITIONAL_NOISE_1("Additional Noise 1", Float) = 0
        _AdditionalNoiseScale_1("Additional Noise Scale", Range(0, 0.1)) = 0
        _AdditionalNoiseColor_1("Additional Noise Color", Color) = (1, 1, 1, 1)
		[Toggle] _ADDITIONAL_NOISE_2("Additional Noise 2", Float) = 0
		_AdditionalNoiseScale_2("Additional Noise Scale 2", Range(0, 0.1)) = 0
		_AdditionalNoiseColor_2("Additional Noise Color 2", Color) = (1, 1, 1, 1)
    }
    ...
    SubShader
    {
        ...
        HLSLINCLUDE
			...
			#pragma shader_feature_local  _INVERT_NOISE_ON
			#pragma shader_feature_local  _ADDITIONAL_NOISE_1_ON
			#pragma shader_feature_local  _ADDITIONAL_NOISE_2_ON
            ...
            CBUFFER_START(UnityPerMaterial)
				...
				float _GrassMinHeightThreshold;
				float _GrassMaxHeightThreshold;
				float _TerrainHeightMultiplier;
				float _GrassDistanceVisibility;
				float _TerrainSlopeThreshold;

				float _NoiseHeightInfluence;
				float _NoiseHeightBias;
                ...
                float _AdditionalNoiseScale_1;
                float4 _AdditionalNoiseColor_1;
				float _AdditionalNoiseScale_2;
				float4 _AdditionalNoiseColor_2;
            CBUFFER_END
			...
			float Noise(float2 coord, float scale, float seed, int nbOctaves){
				float noiseValue = 0.0f;
				for (int i = 0; i < nbOctaves; i++)
				{
					noiseValue += SimplexNoise(coord * scale * pow(2, i) + seed) / pow(2, i);
				}
			...
			{
				// don't draw grass if it's too far away
				float distanceFromCam = distance(TransformObjectToWorld(input[0].vertex.xyz), _WorldSpaceCameraPos);
				if (distanceFromCam > _GrassDistanceVisibility) return;

				float noiseValue = Noise(input[0].vertex.xz, _NoiseScale, 0, _NoiseOctaves);
				#if _INVERT_NOISE_ON
					noiseValue = 1 - noiseValue;
				#endif

				float facingTop = dot(input[0].normal, float3(0, 1, 0)); // avoid grass on side of terrain

				if (noiseValue > _NoiseThreshold &&
					input[0].vertex.y > _GrassMinHeightThreshold * _TerrainHeightMultiplier &&
					input[0].vertex.y < _GrassMaxHeightThreshold * _TerrainHeightMultiplier &&
					facingTop >= _TerrainSlopeThreshold)
				{
					...
					// Scale the height based on the noise value
					height *= lerp(_NoiseHeightBias, 1 + _NoiseHeightInfluence, noiseValue);
...
Pass
{
	...
	float4 frag(GeomData i) : SV_Target
	{
		...
		#if _ADDITIONAL_NOISE_1_ON
			float externalNoiseValue = Noise(i.worldPos.xz, _AdditionalNoiseScale_1, 0.0f, 2);
			tipColor = lerp(tipColor, _AdditionalNoiseColor_1, externalNoiseValue);
		#endif
		#if _ADDITIONAL_NOISE_2_ON
			float externalNoiseValue2 = Noise(i.worldPos.xz, _AdditionalNoiseScale_2, 10000.0, 2);
			tipColor = lerp(tipColor, _AdditionalNoiseColor_2, externalNoiseValue2);
		#endif
		
		float4 color = lerp(_RootColor, tipColor, i.uv.y);
