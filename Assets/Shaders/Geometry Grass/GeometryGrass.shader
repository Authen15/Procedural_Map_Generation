	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			void geom(triangle v2g input[3], inout TriangleStream<g2f> triStream)
			{
				...
				float3 avgNormal = (input[0].normalWS + input[1].normalWS + input[2].normalWS) / 3.0f;
				//Calculate the dot product between the average normal and the up direction
				float dotProduct = dot(avgNormal, float3(0, 1, 0));

				//Check if the dot product is greater than a threshold (e.g., 0.9) to ensure the face isn't vertical
				if (grassVisibility >= _GrassThreshold && dotProduct > 0.9)
				{
					float3 pos = (input[0].positionWS + input[1].positionWS + input[2].positionWS) / 3.0f;
					// Adjust the multiplier `0.5f` as needed
					float3 randomOffset = rand(pos.xyz) * _BladeWidthMax * .5f; 
					pos += randomOffset;
